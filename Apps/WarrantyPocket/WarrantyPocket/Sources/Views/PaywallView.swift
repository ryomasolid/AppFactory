...
/// Pro へのアップグレード画面。買い切りで「登録無制限」「PDF・CSV書き出し」「広告非表示」。
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            VStack(alignment: .leading, spacing: 18) {
                benefit(
                    "square.stack.3d.up.fill", "商品を何件でも",
                    String(localized: "無料は\(ProLimits.freeItems)件まで。家じゅうの家電や持ち物をまとめて管理")
                )
                benefit(
                    "doc.richtext", "PDF・CSVで書き出し",
                    String(localized: "保証書の写真ごとPDFに。引越しや火災保険の請求、家族への共有に")
                )
                benefit("rectangle.slash", "広告を非表示", String(localized: "すっきり快適に"))
            }
            .padding(.horizontal, 32)

            Text("写真の読み取りと、保証終了のお知らせは無料版でも使えます。")
                .font(.footnote)
